...
export default function SafetyVideo({ onVideoWatched, hasWatched }: SafetyVideoProps) {
  const [isPlaying, setIsPlaying] = useState(false);
  const [videoUrl, setVideoUrl] = useState('https://www.youtube.com/embed/A_Zg6UqQylo'); // High quality maritime instruction/harbor style video

  return (
    <div id="safety-video-container" className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
      {/* Header */}
      <div className="bg-slate-900 px-6 py-4 flex items-center justify-between border-b border-slate-850">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-white text-base">Instruções de Segurança</h3>
            <p className="text-xs text-slate-400">Vídeo obrigatório para qualquer visitante</p>
          </div>
        </div>
        {hasWatched ? (
      ...
      <div className="relative flex-1 bg-black aspect-video flex items-center justify-center">
        {!isPlaying ? (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center p-6 bg-gradient-to-t from-slate-950/90 via-slate-900/60 to-slate-950/80">
            ...
            <div className="absolute inset-0 opacity-20 bg-cover bg-center bg-no-referrer" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1578575437130-527eed3abbec?auto=format&fit=crop&w=800&q=80')" }}></div>
            ...
            <div className="relative z-20 flex flex-col items-center">
              ...
              <h4 className="mt-4 font-bold text-lg text-white">Segurança Wilson Sons - Integração Operacional</h4>
              <p className="mt-1 text-sm text-slate-300 max-w-sm">
                Entenda os riscos operacionais, o uso correto dos EPIs obrigatórios e as normas do terminal marítimo.
              </p>
              
              <div className="mt-4 flex gap-4 text-xs text-slate-400">
                <span className="flex items-center gap-1"><FileText className="w-3.5 h-3.5 text-amber-500" /> Duração: ~2 min</span>
      ...
      <div className="p-5 bg-amber-500/5 border-t border-slate-100 flex-1 flex flex-col justify-between">
        <div>
          <h4 className="text-xs font-bold text-amber-800 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <ShieldAlert className="w-4 h-4 text-amber-600" /> Regras Cruciais do Terminal
          </h4>
          <ul className="space-y-2 text-xs text-slate-600">
            <li className="flex items-start gap-2">
              <span className="text-amber-500 font-bold">•</span>
              <span><strong>Acesso às Áreas de Risco:</strong> Apenas pessoas cadastradas, integradas e com crachá visível.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-amber-500 font-bold">•</span>
              <span><strong>Calçado Apropriado:</strong> Completamente proibido chinelos, sapatilhas ou saltos. Exige-se botas de biqueira de aço nas áreas alfandegadas.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-amber-500 font-bold">•</span>
              <span><strong>Roupas Operacionais:</strong> Não use roupas sem mangas (regatas) ou calças curtas (shorts). Calça comprida é obrigatória.</span>
            </li>
          </ul>
        </div>
