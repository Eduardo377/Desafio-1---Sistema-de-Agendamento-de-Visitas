...
export default function App() {
  ...
  const handleCreateSchedule = (formData: Omit<VisitSchedule, 'id' | 'createdAt' | 'status'>) => {
    ...
    triggerToast('✓ Solicitação de Agendamento enviada e gravada com sucesso!');
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans relative antialiased selección-modern">
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold tracking-wider text-amber-600 bg-amber-50 px-2 py-0.5 rounded uppercase">Port & Logística</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                  <span className="text-[10px] text-slate-400 font-mono">MVP Sandbox</span>
                </div>
                <h1 className="text-lg font-bold text-slate-900 leading-tight">Agendamento de Visitas - Wilson Sons</h1>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2 max-w-2xl">
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-amber-500/10 text-amber-400 text-xs font-bold rounded-full border border-amber-500/20 uppercase tracking-widest">
                <Sparkles className="w-3.5 h-3.5" /> Portal de Segurança Integrada
              </span>
              <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Regras de Acesso e Credenciamento de Visitantes</h2>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                Este sistema agiliza a liberação na portaria dos terminais alfandegados da Wilson Sons. 
                Os visitantes devem obrigatoriamente preencher os dados, assistir às diretrizes de segurança 
                e estar em conformidade física antes de receberem o crachá de livre trânsito.
              </p>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 md:max-w-xs shrink-0 backdrop-blur-sm self-start md:self-auto">
              ...
              <p className="text-xs text-slate-300 mt-2 leading-relaxed">
                Desenvolva o checkout visual para o seu trabalho rapidamente. Use a aba "Gerador Low-Code" 
                para copiar os blocos requeridos de forma integral!
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="lg:col-span-5 flex flex-col">
                <SafetyVideo 
                  onVideoWatched={() => {
                    ...
                    triggerToast('Obrigado! Vídeo de segurança validado.', 'info');
                  }} 
      ...
      <footer className="bg-slate-950 border-t border-slate-900 py-8 px-4 sm:px-6 lg:px-8 text-center mt-12">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-orange-600/10 text-orange-500 border border-orange-500/10 flex items-center justify-center font-bold">W</div>
            <span className="text-slate-400 font-medium">Wilson Sons Portal Administrativo</span>
          </div>
          
          <div id="mandatory-footer-container" className="py-2.5 px-4 bg-slate-900 border border-slate-800 rounded-lg text-amber-500 font-semibold tracking-wide shadow-inner text-center">
            Projeto desenvolvido para fins educativos na KODIE Academy
          </div>
          
          <p className="text-slate-500 text-[11px]">&copy; 2026. Todos os direitos reservados para fins de simulação.</p>
        </div>
      </footer>
    </div>
  );
